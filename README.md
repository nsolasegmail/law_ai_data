...
## 📁 **Project Structure & Table Descriptions**

```
harvey/
├── models/
│   ├── dim_user.sql                           # SCD Type 2 user dimension
│   ├── dim_firms.sql                          # SCD Type 2 firm dimension
│   ├── daily_user_engagement.sql              # Daily power user analytics
│   ├── daily_firm_usage_summary.sql           # Daily firm metrics
│   ├── harvey_analytics_dashboard.sql         # Daily platform KPIs
│   └── schema.yml                             # Model documentation & tests
├── macros/
│   ├── engagement_classification.sql           # Engagement level logic
│   ├── usage_pattern_classification.sql        # Usage pattern logic
│   ├── quality_classification.sql              # Quality scoring logic
│   ├── firm_health_classification.sql          # Firm health logic
│   ├── calculate_growth_rate.sql               # Growth calculations
│   ├── date_helpers.sql                        # Date utility functions
│   └── test_helpers.sql                        # Custom testing macros
...
- Access to Harvey's raw data tables
...
### **Daily Power User Analysis:**
```bash
# Run daily incremental job for current month
dbt run --select daily_user_engagement --vars '{"is_incremental": true}'
```
...
WHERE power_user_tier = 'Power User'
ORDER BY power_user_score DESC;
```

### **Power User Growth Trend:**
```sql
SELECT 
  month,
...
SELECT 
  ...
  power_users,
  power_users_rate,
  power_user_penetration_rate,
  power_user_health_indicator
